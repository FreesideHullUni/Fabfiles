...
def dconf(c):
    ...
    c.append('/etc/dconf/profile/gdm', 'user-db:user\n'
           'system-db:gdm\n'
           'file-db:/usr/share/gdm/greeter-dconf-defaults', use_sudo=True)
    c.sudo('mkdir /etc/dconf/db/gdm.d')
    c.append('/etc/dconf/db/gdm.d/00-login-screen', '[org/gnome/login-screen]\n'
...
def deploy_ff_policy(c):
    with cd('/usr/lib64/firefox/'):
        c.put('distFiles/firefox/distribution.ini', 'distribution/',
            use_sudo=True)


@task
def wol_setup(c):
    c.append('/etc/sysconfig/network-scripts/ifcfg-eno1',
